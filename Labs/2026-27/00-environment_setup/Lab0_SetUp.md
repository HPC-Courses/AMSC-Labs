...
Beware of the installation path (use `pwd` and save the path somewhere), i.e. pay attention at the folder in which the container is pulled.
...
To enable `mk` modules, you should type each time you start a shell inside the container
```
...
To start a shell inside the container, use the command
```bash
docker run -it --name amsc -v /path/to/shared-folder:/home/ubuntu/shared-folder quay.io/pjbaioni/amsc_mk:2025 /bin/bash
```
where you must replace `/path/to/shared-folder` with the actual path of a folder on your MacOS machine that you want to share with the Docker container (e.g. `/Users/matteo/polimi/amsc26-27/shared-folder`). This folder will be mounted inside the container at path `/shared-folder`. 
...
**WARNING:** to avoid problems `/path/to/shared-folder` should not contain white spaces or special characters. 
...
### 3.4. Using the libraries inside the container

Look at [Section 2.4](#24-using-the-libraries-inside-the-container).
...
To test possible issues with MPI, using a text editor, create a file `test_mpi.cpp` with content:
...
int main(int argc, char** argv) {
    ...
    int valore = (rank == 0) ? 42 : 0;
    MPI_Bcast(&valore, 1, MPI_INT, 0, MPI_COMM_WORLD);
    if (valore != 42) {
        std::cerr << "Rank " << rank << ": BCAST FAILED\n";
        MPI_Abort(MPI_COMM_WORLD, 1);
    }

    int contributo = rank + 1;
    int somma = 0;
    MPI_Reduce(&contributo, &somma, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);

    if (rank == 0) {
        const int atteso = size * (size + 1) / 2;
        std::cout << "\nReduce: sum = " << somma
                  << " (expected " << atteso << ") -> "
                  << (somma == atteso ? "OK" : "ERROR") << std::endl;
    }
...
From inside the terminal move to the folder containing the file `test_mpi.cpp`. Compile and run the previous script by typing:

```bash
mpic++ -std=c++20 -O2 test_mpi.cpp -o test_mpi
