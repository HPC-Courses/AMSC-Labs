...
main(int argc, char **argv)
{
  // Vector problem.
  {
...

    {
      ...
      auto jac_full = make_jacobian<FullJacobian>(jacobian_fun);
			//hand-made factory:
			//std::unique_ptr <JacobianBase> jac_full = std::make_unique <FullJacobian> (jacobian_fun);

      Newton newton(system, std::move(jac_full)); // && as const &
...

    {
      ...
      auto jac_discrete = make_jacobian<DiscreteJacobian>(system, h);
