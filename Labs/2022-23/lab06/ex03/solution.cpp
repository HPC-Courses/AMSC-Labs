...
inline double simpson_multithreaded(
...
int main(int argc, char** argv) {
  // parse arguments
  // - the number of intervals in which [0,1] is split into when evaluating the integral
  // - the number of OpenMP threads we want to use
  if (argc != 3) {
    std::cerr << "Usage: " << argv[0] << " number-of-intervals number-of-threads\n";
  ...
  const double local_integral = simpson_multithreaded(
    [](auto x){ return std::sqrt(1 - x*x); }, 
    a_local, a_local + local_interval_size, n_local, nthreads);
  double integral = 0.0;
  // the value of the integral is the sum on the integrals on the partitions of [0, 1]
