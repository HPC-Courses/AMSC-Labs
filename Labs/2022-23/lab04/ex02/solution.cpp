...
std::array<double, N> compute_gradient(
  ...
  // broadcast result, each proc has all the gradient
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    if(belongs_to_proc /*!= 0*/) {
      if(rank == belongs_to_proc){
        ...
        MPI_Recv (&gradient[i], 1, MPI_DOUBLE, belongs_to_proc, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      }
    } 
  }
  return gradient;
...
int main(int argc, char* argv[]) {
  ...
  // define the field f of which we compute the gradient at the point where we evaluate it
  const Function f = [](auto v) {
     return std::sin(v[0]) + std::exp(v[1]) + std::cos(v[2]) + 2*v[3]*v[3]*v[3]; 
  };
  ...
  if(rank == 0) {
    ...
    for(size_t i = 0; i < g.size(); ++i) {
      ...
      std::cout << i << " | "<< g[i] << (err < 1e-6 ? " PASS" : " FAIL") << std::endl;
    }
  }
  MPI_Finalize();
  return 0;
