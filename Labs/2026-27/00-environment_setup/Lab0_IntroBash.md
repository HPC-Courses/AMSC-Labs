...
### What is `Bash`?
`Bash` stands for: `Bourne Again Shell`, a homage from its creator Brian Fox (GNU foundation) to Stephen Bourne (the inventor of sh) . It is the default shell for most Unix systems and Linux distributions. It is both a command interpreter and a scripting language. The shell might be changed by simply typing its name and even the default shell might be changed for all sessions.
...
- **login**:
        - `/etc/profile`, `~/.bash_profile`, `~/.bash_login`, and `~/.profile` for `Bash`
...
-  `env | grep Name_of_Variable` to visualize the value of the environmental variable "Name_of_Variable" (see below for `|` and `grep` commands)
